...
- **[`vercel.json`](vercel.json)** — `installCommand` is a **POSIX `sh -c` one-liner** (`uv pip install --system` or `pip install --break-system-packages`), then **`npm ci`** in `dashboard/`. **`buildCommand`** runs [`scripts/vercel-build.sh`](scripts/vercel-build.sh) (firmware ELF + dashboard build with `VITE_API_URL=relative`). The UI is copied to **`bridge/_vercel_public/`** (what FastAPI serves) and to **`public/`** (so a project **Output Directory** of **`public`** still finds files after the build). Do **not** add **`"outputDirectory": "public"`** to **`vercel.json`**: that pattern deploys **static-only**, and **`/pa/*`** and **`/health`** return edge **`NOT_FOUND`**. [`scripts/vercel-install.sh`](scripts/vercel-install.sh) mirrors install logic for local checks.
...
2. **Output Directory:** Prefer **empty / default** in the Vercel dashboard so routing is obvious. If it is set to **`public`**, the build still creates **`public/`** so the build passes; **`vercel.json` must not** set **`outputDirectory`** (that is what triggered static-only before). After deploy, **`curl …/health`** must return JSON; if you still get **`NOT_FOUND`**, clear **Output Directory** in project settings and redeploy.
