...
- **[`vercel.json`](vercel.json)** — **`"framework": "fastapi"`** so the project is not mis-detected as a static/Vite export. **`functions.app.py.includeFiles`** forces the **`firmware/bin/protocol_analyzer`** ELF and **`bridge/_vercel_public/**`** into the serverless bundle (Python import tracing does not pick up those paths, so without this you get **`FUNCTION_INVOCATION_FAILED`** / missing binary at runtime). `installCommand` is a **POSIX `sh -c` one-liner** (`uv pip install --system` or `pip install --break-system-packages`), then **`npm ci`** in `dashboard/`. **`buildCommand`** runs [`scripts/vercel-build.sh`](scripts/vercel-build.sh) (firmware ELF + dashboard build with `VITE_API_URL=relative`). The UI is copied to **`bridge/_vercel_public/`** (what FastAPI serves for **`/`** and **`/assets`**). **`public/`** only receives **`assets/`** (and other non-SPA root files), **not** `index.html` / `404.html`, so Vercel still validates an Output Directory of **`public`** without turning the deployment into **CDN-only** (which yields edge **`NOT_FOUND`** on **`/health`** and **`/pa/*`**). Do **not** add **`"outputDirectory": "public"`** to **`vercel.json`**. [`scripts/vercel-install.sh`](scripts/vercel-install.sh) mirrors install logic for local checks.
