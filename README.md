...
- **[`vercel.json`](vercel.json)** — **`"framework": "fastapi"`** so the project is not mis-detected as a static/Vite export. The Vercel **`functions`** glob in **`vercel.json` only matches files under `api/`**, so this repo does **not** use `functions` for the root FastAPI app. Instead, [`scripts/vercel-build.sh`](scripts/vercel-build.sh) copies **`protocol_analyzer`** into **`bridge/protocol_analyzer`** next to **`api.py`** so it ships with the **`bridge`** package bundle, and the Vite build lives under **`bridge/_vercel_public/`** (not gitignored, so it is present after the build for packaging). `installCommand` is a **POSIX `sh -c` one-liner** (`uv pip install --system` or `pip install --break-system-packages`), then **`npm ci`** in `dashboard/`. **`buildCommand`** runs [`scripts/vercel-build.sh`](scripts/vercel-build.sh) (firmware ELF + dashboard build with `VITE_API_URL=relative`). **`public/`** only receives **`assets/`** (and other non-SPA root files), **not** `index.html` / `404.html`, so Vercel still validates an Output Directory of **`public`** without turning the deployment into **CDN-only** (which yields edge **`NOT_FOUND`** on **`/health`** and **`/pa/*`**). Do **not** add **`"outputDirectory": "public"`** to **`vercel.json`**. [`scripts/vercel-install.sh`](scripts/vercel-install.sh) mirrors install logic for local checks.
...
3. **Output Directory:** Prefer **empty / default**. If it must be **`public`**, the build keeps **`public/`** non-empty **without** copying **`index.html`** there (see above). **`vercel.json` must not** set **`outputDirectory": "public"`**. After deploy, **`curl …/health`** must return **`{"status":"ok"}`**.
...
**“NOT_FOUND” / “The page could not be found”** in the UI after **Analyze** usually means the request never reached FastAPI (Vercel edge **`NOT_FOUND`**). Typical causes: **Framework preset** not **FastAPI**, **`index.html` in `public/`** with a static output directory (CDN-only), **`"outputDirectory": "public"`** in **`vercel.json`**, **Install Command** override, or wrong **Root Directory**. **`curl https://<your-app>.vercel.app/health`** should return **`{"status":"ok"}`**; if it is **`NOT_FOUND`**, fix the preset/output directory first.
